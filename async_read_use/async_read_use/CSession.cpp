...
void CSession::Start(){
	...
	boost::asio::async_read(_socket,boost::asio::buffer(_recv_head_node->_data,HEAD_LENGTH),
		bind(&CSession::HandleReadHead,this,placeholders::_1,placeholders::_2,SharedSelf()));
}
...
void CSession::HandleReadHead(const boost::system::error_code& error, size_t bytes_transferred, std::shared_ptr<CSession> shared_self){
	if (!error) {
		...
		data_len = ntohs(data_len); // 假设网络字节序是大端序，转换为本地字节序
		...
		boost::asio::async_read(_socket, boost::asio::buffer(_recv_msg_node->_data,
			_recv_msg_node->_total_len), bind(&CSession::HandleReadMsg, this,
				placeholders::_1, placeholders::_2, SharedSelf()));
	}else {
